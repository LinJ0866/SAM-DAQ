...
########################maxF##############################
        ...
        precision = targetHist / (targetHist + nontargetHist + 2e-16)
        recall = targetHist / np.sum(gt)
        return precision, recall, meanF

    def show(self):
        precision = np.mean(self.precision, axis=0)
        recall = np.mean(self.recall, axis=0)
        fmeasure = 1.3 * precision * recall / (0.3 * precision + recall + 2e-16)
        fmeasure_avg = np.mean(self.meanF, axis=0)
        return fmeasure.max(), fmeasure_avg, precision, recall
...
class cal_sm(object):
    ...
    def s_object(self, in1, in2):
        ...
        return 2 * x / (pow(x, 2) + 1 + sigma_x + 2e-16)
    ...
    def ssim(self, in1, in2):
        ...
        sigma_x = np.var(in1)
        sigma_y = np.var(in2)
        sigma_xy = np.sum(np.sum((in1 - x) * (in2 - y))) / (N - 1 + 1e-8)
